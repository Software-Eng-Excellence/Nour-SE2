...
import { SQLiteCake, SQLiteCakeMapper } from "../../mapper/Cake.mapper";
...
const UPDATE_ID = `UPDATE ${tableName} SET
        type = ?, flavor = ?, filling = ?, size = ?, layers = ?, frostingType = ?, 
        frostingFlavor = ?, decorationType = ?, decorationColor = ?, customMessage = ?, 
        shape = ?, allergies = ?, specialIngredients = ?, packagingType = ? 
        WHERE id = ?`;
...
export class CakeRepository implements IRepository<IdentifiableCake> , Initializable {
    ...
    async get(id: id): Promise<IdentifiableCake> {
        try{
            ...
            if(!result){
                throw new ItemNotFoundException("Cake not found with id: " + id  +" Not found");
            }
            return new SQLiteCakeMapper().map(result);
        }catch(error:unknown){
            logger.error("Failed to get cake: %s %o", id, error as Error);
            throw new DbException("Failed to get cake: "+ id, error as Error);
        }
    }
    ...
    async update(item: IdentifiableCake): Promise<void> {
        try{
            ...
            logger.error("Failed to update cake: %s %o", item.getID(), error as Error);
